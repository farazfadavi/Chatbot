...
class Chatbot:
    ...
    punc= [",","!",".",";","?","but","yet"]
    negationWords = ['not','nor',"n't","didn't","wasn't","don't","dont","didnt","wasnt"]
      ...
      if self.is_turbo == True:
        ...
        if self.askAboutMovies:
            ...
            if movies and not sentiment:
                response += "I am not sure what your opinion about %s. Can you clarify?" % self.conjPhraseFromList(movies) 
            elif movies:
                ...
                if len(movies)>1:
                    moviesString = ", ".join(["\"%s\""%movie for movie in movies[:-1]])+" and \"%s\""%movies[-1]
                else:
                    moviesString = "\"%s\""%movies[0]
                
                # According to the sentiment, add a verb to the response
                if sentiment>0: dont='like'
                else: dont="don't like"
                if dont:
                    response += '%s %s %s.' %(random.choice(['Ok, so you','You','So you','Ok, you']),dont,moviesString)
                    response += unkString
                else:
        ...
        if self.askAboutGenres:
            ...
            if movies and not sentiment:
                response += "I am not sure what your opinion about %s. Can you clarify?" % self.conjPhraseFromList(genres) 
    ...
    def isolateMovieNames(self,line):
        ...
        movies = [find for find in re.findall(self.quatedMovieNamePatter,line)]
    ...
    def conjPhraseFromList(self,lst):
        if len(lst)>1:
            string = ", ".join(["\"%s\""%movie for movie in lst[:-1]])+" and \"%s\""%lst[-1]
        else:
            string = "\"%s\""%lst[0]
        return string
    ...
    def handleArticle(self,title):
        ...
        newTitles = []
        for title in titles:
            find = re.findall(", (l'|de|di|der|das|die|la|le|the|an|a)",title.lower())
            if find:
    ...
    def read_data(self):
      ...
      for title in self.titlesPlain: 
          self.titlesWithoutYear=self.titlesWithoutYear.union(self.handleArticle(title[:-7]))
          #if len(self.handleArticle(title[:-7]))>1: print self.handleArticle(title[:-7])
