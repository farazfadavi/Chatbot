...
class Chatbot:
    ...
    def process(self, input):
        ...
        if self.is_turbo == True:
            ...
            if self.askAboutMovies:
                ...
                # TODO if no sentiment given, still check if movie is in the database
                print "Movies:", movies
                if movies and not sentiment:
                    response += "I am not sure what your opinion about %s. Can you clarify?" % self.conjPhraseFromList(
                        movies)
                elif movies:
    ...
    def conjPhraseFromList(self, lst):
        if len(lst) > 1:
            string = ", ".join(["\"%s\"" % movie for movie in lst[
                               :-1]]) + " and \"%s\"" % lst[-1]
        else:
            string = "\"%s\"" % lst[0]
        return string
